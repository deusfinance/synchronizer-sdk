...
const fetchConducted = async (providerMapping, chainId) => {
  try {
    if (!providerMapping) {
    ...
    let responses = await Promise.allSettled(promises); // Modify output to default format to ensure easier cross-matching
    ...
    let modifiedResponses = responses.map(response => {
      ...
      let tokens = (response === null || response === void 0 ? void 0 : (_response$value = response.value) === null || _response$value === void 0 ? void 0 : _response$value.tokens) ?? [];
    ...
    let conductedWithConsensus = modifiedResponses.reduce((acc, providerResponse, index) => {
      for (const symbol in providerResponse) {
        ...
        const hasMatchingShort = acc[symbol]['short'] === shortValue; // All the nodes must provide a response for this symbol, else its an anomaly as well
